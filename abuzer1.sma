#define VERSION	"3.00 Beta 1 Persians Edit"
...
#define DEFAULT_PMODEL	    "models/player/normal/normalzombi.mdl"
#define DEFAULT_WMODEL	    "models/v_normalzombi_el.mdl"
...
public menuyuduzenle(id) {
    ...
    menu_additem(generBiomenu , "Jet \r[\yHiz ve Defans\r] \r[\ySpeed And Defence\r]", "2", 0)
...
{
	set_hudmessage(0, 255, 0, 0.03, 0.93, _, 0.2, 0.2)
	...
	for(id = 1; id <= g_maxplayers; id++) if(is_user_alive(id) && !is_user_bot(id) && g_zombie[id])
	{
		...
		if(g_classcount > 1)
			ShowSyncHudMsg(id, g_sync_hpdisplay, "Saglik: %0.f  Sinif: %s (%s)", health, g_class_name[class], g_class_desc[class])
		else	
			ShowSyncHudMsg(id, g_sync_hpdisplay, "Saglik: %0.f", health)
	}
}
...
public task_initround()
{
	...
	{
		set_dhudmessage(0, 255, 0, -1.0, 0.35, 2, 6.0, 5.0, 0.1, 0.8)
		show_dhudmessage(0, "[LasTShoT]^n Zombiler Geliyor !")
