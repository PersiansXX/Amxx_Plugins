#define VERSION	"Persians 4.3 Alfa 3 Remake Fix"
...
#define DEFAULT_PMODEL	    "models/player/Normal_Zombi/Normal_Zombi.mdl"
#define DEFAULT_WMODEL	    "models/player/v_2025_Normal.mdl"
...
public plugin_precache()
{
	...
	{
		formatex(buffer, sizeof(buffer), "models/player/%s/%s.mdl", admin_model[i], admin_model[i])
...
public menuyuduzenle(id) {
    ...
    menu_additem(generBiomenu , "Jet \r[\yHiz ve Defans\r] \r[\yFast And Defence\r]", "2", 0)	
    menu_additem(generBiomenu , "Hulk \r[\yGuc ve Defans\r] \r[\yStrong And Defence\r]", "3", 0)
    menu_additem(generBiomenu , "ZipZip \r[\yZiplama ve Defans\r] \r[\yJump And Defence\r]", "4", 0)
    menu_additem(generBiomenu , "Rejenerasyon \r[\yCan ve Defans\r] \r[\yHealth And Defence\r]", "5", 0)
...
public task_showtruehealth()
{
	...
	for(id = 1; id <= g_maxplayers; id++) 
	{
		...
		if(is_user_alive(id) && !is_user_bot(id) && g_zombie[id])
		{
			...
			ShowSyncHudMsg(id, g_sync_hpdisplay, "[Health: %i | LS Coin: %i | Class: %s]", health, g_coin[id], g_class_name[class])
		} else if(is_user_alive(id) && !is_user_bot(id) && !g_zombie[id]) {
			ShowSyncHudMsg(id, g_sync_hpdisplay, "[Health: %i | Armor: %i | LS Coin: %i]", health, armor, g_coin[id])
		}
	}
}
...
public fm_reset_user_model(id)
{
	...
	{
	fm_set_user_model(id, admin_model[random_num(0, charsmax(admin_model))])
	}
	else
	{
